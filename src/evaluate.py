...
def top_n_capture(y_true, probs, frac=0.2):
    order=probs.argsort()[::-1]; cutoff=max(1, int(len(probs)*frac)); idx=order[:cutoff]
...
def main():
    ...
    scored=test_df.copy(); scored["risk_score"]=probs; scored.to_csv("outputs/tables/test_scored.csv", index=False)
    for group_col, table in run_fairness_suite(scored, prob_col="risk_score", target_col=TARGET_COLUMN).items():
        table.to_csv(f"outputs/tables/fairness_{group_col}.csv", index=False)
    calib=calibration_table(y_test, probs); calib.to_csv("outputs/tables/calibration_table.csv", index=False)
    fpr,tpr,_=roc_curve(y_test, probs); plt.figure(); plt.plot(fpr,tpr); plt.xlabel("False Positive Rate"); plt.ylabel("True Positive Rate"); plt.title("ROC Curve"); plt.savefig("outputs/figures/roc_curve.png", bbox_inches="tight"); plt.close()
    precision,recall,_=precision_recall_curve(y_test, probs); plt.figure(); plt.plot(recall, precision); plt.xlabel("Recall"); plt.ylabel("Precision"); plt.title("Precision-Recall Curve"); plt.savefig("outputs/figures/pr_curve.png", bbox_inches="tight"); plt.close()
    plt.figure(); plt.plot(calib["predicted_probability"], calib["observed_rate"], marker="o"); plt.plot([0,1],[0,1], linestyle="--"); plt.xlabel("Predicted Probability"); plt.ylabel("Observed Rate"); plt.title("Calibration Curve"); plt.savefig("outputs/figures/calibration_curve.png", bbox_inches="tight"); plt.close()
