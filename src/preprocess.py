...
def build_preprocessor(df: pd.DataFrame, target_col:str=TARGET_COLUMN):
    ...
    categorical_observed=[c for c in categorical_cols if c not in categorical_all_missing]

    transformers=[]
    if numeric_observed:
        transformers.append(("num", Pipeline([("imputer", SimpleImputer(strategy="median"))]), numeric_observed))
    if numeric_all_missing:
        transformers.append(("num_missing", Pipeline([("imputer", SimpleImputer(strategy="constant", fill_value=0, keep_empty_features=True))]), numeric_all_missing))
    if categorical_observed:
        transformers.append(("cat", Pipeline([("imputer", SimpleImputer(strategy="most_frequent")),("onehot", OneHotEncoder(handle_unknown="ignore"))]), categorical_observed))
    if categorical_all_missing:
        transformers.append(("cat_missing", Pipeline([("imputer", SimpleImputer(strategy="constant", fill_value="missing", keep_empty_features=True)),("onehot", OneHotEncoder(handle_unknown="ignore"))]), categorical_all_missing))
...
def transform_with_feature_names(df: pd.DataFrame, preprocessor)->pd.DataFrame:
    ...
    if hasattr(Xt, "tocoo"):
        return pd.DataFrame.sparse.from_spmatrix(Xt, index=df.index, columns=feature_names)
